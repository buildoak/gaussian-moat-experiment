...
**Project deployment `(R_inner = 80 · 10⁶, R_outer = 80 008 192, K ∈ {36, 40})`** (computed by `build/bz_check.py` in 50-digit arbitrary precision; see `docs/supportive/2026-04-20-codex-bz-check-design.md`):

- `K = 36`: `BZ_I` candidates are `6 400 000 960 000 035` and `6 400 000 960 000 036`; `BZ_O` candidate is `6 401 309 827 010 596`. All three are non-Gaussian-prime norms (`6 400 000 960 000 035` is composite; `6 400 000 960 000 036 = 4 · 40 000 003²`; `6 401 309 827 010 596 = 80 008 186²`).
- `K = 40`: `BZ_I` candidate is `6 400 001 011 928 891`; `BZ_O` candidate is `6 401 309 775 076 432`. Both are composite non-Gaussian-prime norms.

Both project parameter sets are BZ-prime-free, so `geo_I_w = geo_I` and `geo_O_w = geo_O`. Future deployments must re-run `build/bz_check.py`; the equality is per-deployment, not a theorem of the interval form alone.
