...
Whether a moat is a total barrier or origin-only depends on the local percolation parameters:

| $k^2$ | $R_{\text{moat}}$ | Backward offsets | Density $\rho$ | $E[\text{backward degree}]$ | $E[\text{total degree}]$ | Moat type |
|--------|-----|------|------|-------|-------|-----------|
| 2 | $\sim 12$ | 2 | 0.225 | 0.45 | 0.90 | Total barrier |
| 4 | $\sim 45$ | 2 | 0.153 | 0.31 | 0.61 | Total barrier |
| 26 | $\sim 1.02\text{M}$ | 22 | 0.045 | 0.99 | 1.98 | Origin-only |
| 32 | $\sim 2.82\text{M}$ | 26 | 0.042 | 1.09 | 2.18 | Total barrier |
| 36 | $\sim 80\text{M}$ | 26 | 0.034 | 0.89 | 1.79 | Total barrier |
| 40 | $\sim 1.05\text{B}$ | 30 | 0.030 | 0.90 | 1.81 | Expected total |

**Interpretation.** At $k^2 = 26$, the expected total degree at the moat radius is approximately 2.0, placing the graph in the supercritical regime of site percolation on a random geometric graph. In this regime, large-scale connected components freely bridge any finite-width annular band. Empirically, a 30-unit-wide band at the Tsuchimura moat for $k^2 = 26$ contains 41 independent spanning components. Fat stripe reports "not blocked" for such moats, correctly reflecting that the annulus is not a total barrier.

At $k^2 = 32$ and $k^2 = 36$, the total degree drops to 1.8--2.2 at the moat radius. The graph is sufficiently sparse that large-scale fragmentation prevents *any* component from spanning the annulus, not merely the origin component. Fat stripe detects these as total barriers.
...
The percolation parameters at the expected moat radius for $k^2 = 40$ ($R \approx 1.05\text{B}$) closely resemble those at the confirmed $k^2 = 36$ moat:

| Parameter | $k^2 = 36$ at $R = 80\text{M}$ | $k^2 = 40$ at $R = 1.05\text{B}$ |
|-----------|------|------|
| Local prime density $\rho$ | 0.034 | 0.030 |
...
At $k^2 = 36$, the percolation boundary ($f(r) = 0$) was found at $R \approx 80.4\text{M}$ with empirical prime density $\rho \approx 3.50\%$. The mean degree at the transition:

$$
d_c = \rho \times z = 0.0350 \times 112 = 3.92.
$$

Assuming constant critical mean degree across $k^2$ values:

$$
\rho_c(k^2 = 40) = \frac{d_c}{z} = \frac{3.92}{128} = 0.0306 = 3.06\%.
$$

Setting $\rho(R) = C / (2\ln R) = \rho_c$:

$$
\ln R = \frac{C}{2\rho_c} = \frac{1.274}{2 \times 0.0306} = 20.82, \qquad R = e^{20.82} \approx 1.10 \times 10^9.
$$

This central estimate of $R \approx 1.1\text{B}$ is consistent with the empirical sigmoid midpoint $R_{0.5} \approx 839\text{M}$ (the ISE measures local percolation, which transitions earlier than the mean-degree prediction).
