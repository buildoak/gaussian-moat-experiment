...
The near-identical total degree ($1.79$ vs.\ $1.81$) and similar backward degree suggest that $k^2 = 40$ at its moat radius occupies the same percolation regime as $k^2 = 36$: sufficiently sparse for large-scale fragmentation to create a total barrier.  This prediction is now confirmed by the fat-stripe probes of Section~\ref{sec:fs-k40}: five probes across $R \in [1.03\text{B}, 1.09\text{B}]$ show zero spanning components, consistent with the ISE sigmoid ($\Rhalf \approx 839\text{M}$) predicting complete fragmentation by $R \approx 1\text{B}$.
