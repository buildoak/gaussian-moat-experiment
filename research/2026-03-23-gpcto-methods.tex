...
\toprule
$k^2$ & $R_{\text{moat}}$ & Bkwd offsets & Density $\rho$ & $E[\text{bkwd deg}]$ & $E[\text{total deg}]$ & Moat type \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Percolation parameters and moat type classification at known and estimated moat radii.}
...
At $k^2 = 32$ and $k^2 = 36$, the total degree drops to $1.8$--$2.2$ at the moat radius.  The graph is sufficiently sparse that large-scale fragmentation prevents \emph{any} component from spanning the annulus, not merely the origin component.  Fat stripe detects these as total barriers.
...
The calibration results of Section~\ref{sec:fs-calibration} demonstrate that fat stripe correctly identifies total barriers at known moat radii, but they tile only a sub-sector of the first octant.  A rigorous moat proof requires full-octant coverage (Lemma~\ref{lem:fs-octant}).  For the calibration runs, the sub-sector verdicts are sufficient because the known moats at $k^2 = 32$ and $k^2 = 36$ have been independently verified by Tsuchimura's exhaustive method; the fat-stripe calibration confirms methodological correctness, not new moat existence.  For future UB campaigns at $k^2 = 40$, full-octant tiling is required.
\end{remark}

\subsection{Implications for \texorpdfstring{$k^2 = 40$}{k\texttwosuperior\ = 40}}
...
\subsection{\texorpdfstring{$k^2 = 40$}{k\texttwosuperior\ = 40} results}
...
At $k^2 = 36$, the percolation boundary ($\fr = 0$) was found at $R \approx 80.4\text{M}$ with empirical prime density $\rho \approx 3.50\%$.  The mean degree at the transition:
\[
...
\]
This central estimate of $R \approx 1.1\text{B}$ is consistent with the empirical sigmoid midpoint $\Rhalf \approx 839\text{M}$ (the ISE measures local percolation, which transitions earlier than the mean-degree prediction).
