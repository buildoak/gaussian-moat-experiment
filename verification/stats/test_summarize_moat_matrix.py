...
class MoatMatrixSummaryTest(unittest.TestCase):
    def test_all_moat_hardening_matrix_passes_monotonicity(self) -> None:
        summary = summarize(
        ...
        self.assertTrue(all(item["hardening"] for item in summary["rows"]))
    ...
    def test_optional_k37_to_k39_rows_are_telemetry_not_hardening(self) -> None:
        summary = summarize([row(17000, "MOAT"), row(18000, "SPANNING", k_sq=37)])

        self.assertEqual(summary["monotonicity_status"], "PASS")
        self.assertEqual(summary["findings"], [])
        by_k = {item["k_sq"]: item for item in summary["rows"]}
        self.assertTrue(by_k[36]["hardening"])
        self.assertFalse(by_k[37]["hardening"])
