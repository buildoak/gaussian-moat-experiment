...
- `W=524288` remains trivially spanning through `799M`, so the next useful
  below-850M search band is `800M-849M`.
...
## Phase A - Primary W524288 Fine Ladder
...
```text
805M-830M: 900s per row
835M-840M: 1800s per row
845M-849.5M: 3600s per row
```
...
- stop Phase A on the first `late_timeout_candidate`,
...
```text
W=786432
R_inner = 750M, 800M, 825M, 840M, 848M, 849.5M
timeouts = 1800s, then 3600s for rows >=840M
...
```text
W=1048576
R_inner = 700M, 780M, 820M, 840M, 848M
timeouts = 3600s per row
same BZ, artifact, and stop rules
```

Reason for this order: `W=524288` already has a clean lower ladder through
`799M`, so it is the cheapest way to test the remaining below-850M band. Wider
families are useful only after that band fails to produce pressure.
...
- The scout burns a full day on non-acceptance rows.
  - Use bounded row timeouts and stop on the first candidate signal.
