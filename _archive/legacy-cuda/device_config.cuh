...
//
// Compile with -DTARGET_A100 or -DTARGET_4090 for device-specific tuning,
// otherwise defaults to Jetson Orin Nano parameters.
//
// Usage in CMake:
//   cmake -DTARGET_DEVICE=a100 ..   -> adds -DTARGET_A100
//   cmake -DTARGET_DEVICE=4090 ..   -> adds -DTARGET_4090
//   cmake -DTARGET_DEVICE=jetson .. -> default (no define)
...
#elif defined(TARGET_4090)
  // NVIDIA RTX 4090: SM 8.9 (Ada Lovelace), 128 SMs, 16384 CUDA cores, 24GB GDDR6X
  // More SMs than A100, same-gen shared mem as Jetson (SM 8.x family).
  // Conservative segment span — 4090 has less VRAM than A100 but more SMs.
  #define GRID_CAP                4096
  #define DEVICE_SEGMENT_SPAN     (1u << 19)   // 512K norms per segment, 32KB shared mem
  #define DEVICE_BITMAP_WORDS     8192u         // 512K/2 = 256K odd slots, /32 = 8192 words
  #define DEVICE_BITMAP_BYTES     (DEVICE_BITMAP_WORDS * 4u)
  ...
  #define THERMAL_CHECK_ENABLED   0
  #define USE_MANAGED_MEMORY      0
  #define SEGMENTS_PER_BATCH_BASE 10000
