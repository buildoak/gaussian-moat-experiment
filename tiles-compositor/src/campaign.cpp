...
int main(int argc, char** argv) {
    ...
    if (use_cuda) {
        ...
        if (num_towers > 0) {
            ...
            for (uint32_t r = 0; r < tpt0; ++r) {
                ...
                if (result.tileop.bytes[0] == OVERFLOW_SENTINEL) {
                    ...
                    std::memset(tower_buf0.data() + static_cast<std::size_t>(r) * TILEOP_SIZE,
                                0, TILEOP_SIZE);
                    tower_buf0[static_cast<std::size_t>(r) * TILEOP_SIZE] = EMPTY_OFFSET;
                }
            }
        ...
        if (!spanning_found && num_towers > 1) {
            ...
            for (int32_t burst_start = 0; burst_start < remaining_towers && !spanning_found;
                ...
                for (int32_t bi = 0; bi < towers_in_burst; ++bi) {
                    ...
                    // Check for overflow sentinels in this tower and replace
                    // with empty TileOps (BUG-2 FIX)
                    for (uint32_t r = 0; r < tpt; ++r) {
                        const std::size_t tile_off =
                            (static_cast<std::size_t>(offset) + r) * TILEOP_SIZE;
                        if (output_buf[tile_off] == OVERFLOW_SENTINEL) {
                            overflow_count++;
                            std::fprintf(stderr,
                                "warning: overflow tile at tower %d row %u "
    ...
    const int32_t towers_processed = spanning_found ? (spanning_tower + 1) : num_towers;
